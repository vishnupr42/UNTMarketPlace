  ...
  start: function () {
    ...
    $('#bidding').submit(function (event) {
      ...
      EcommerceStore.deployed().then(function (i) {
        i.bid(parseInt(productId), sealedBid, {
          ...
          function (f) {
            $('#msg').html('Your bid was successfully submitted!')
            $('#msg').show()
      ...
      EcommerceStore.deployed().then(function (i) {
        i.revealBid(parseInt(productId), web3.toWei(amount).toString(), secretText, {
          ...
          .catch(e => {
            ...
            $('#msg').show()
            $('#msg').html('The reveal has failed. Incorrect info.')
          })
      })
      event.preventDefault()
    })
    ...
    $('#finalize-auction').submit(function (event) {
      ...
      EcommerceStore.deployed().then(function (i) {
        i.finalizeAuction(parseInt(productId), { from: web3.eth.accounts[2], gas: 4400000 }).then(
          function (f) {
            $('#msg').show()
            $('#msg').html('Auction over. Winner declared. ')
            console.log(f)
            location.reload()
          }
        ).catch(function (e) {
          console.log(e)
          $('#msg').show()
          $('#msg').html('Auction has to be finalized by arbiter')
        })
      })
      event.preventDefault()
    })
    ...
    $('#release-funds').click(function () {
      ...
      EcommerceStore.deployed().then(function (f) {
        $('#msg').html('Your transaction was submitted, wait a few seconds').show()
    ...
    $('#refund-funds').click(function () {
      let productId = new URLSearchParams(window.location.search).get('id')
      EcommerceStore.deployed().then(function (f) {
        $('#msg').html('Your transaction submitted, wait a few seconds.').show()
  ...
  EcommerceStore.deployed().then(i => {
    i.addProductToStore(
      ...
      .then(f => {
        ...
        $('#msg').show()
        $('#msg').html('Your product was successfully added to the marketplace')
      })
  })
}
